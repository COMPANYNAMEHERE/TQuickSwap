...
public class TQuickSwapMod implements ModInitializer {
    ...
    @Override public void onInitialize() {
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            DualStore.LoadedProfile snapshot = store.load(p.getUUID(), last);
            LangHelper.ensureLanguageAvailable(server, null);
            if (!snapshot.isEmpty()) {
                ProfileOps.apply(p, snapshot.data());
        ...
        ServerLifecycleEvents.SERVER_STOPPING.register(server -> DualStore.flushAll());
    }
